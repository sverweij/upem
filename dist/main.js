...
function constructSuccessMessage(pOutdatedList) {
    ...
    if (lUpdated.length > 0) {
        lReturnValue += `Up'em just updated these outdated dependencies in package.json:${EOL}${EOL}${lUpdated
            .map((pOutdatedEntry) => `${pOutdatedEntry.package.padEnd(lMaxPackageLength)}${pOutdatedEntry.current.padEnd(lMaxCurrentLength)} -> ${pOutdatedEntry.target.padEnd(lMaxTargetLength)}${pOutdatedEntry.type?.padEnd(lMaxTypeLength) ?? ""} (policy: ${pOutdatedEntry.policy})`)
            .join(EOL)}${EOL}${EOL}`;
    }
    const lNotUpdated = pOutdatedList.filter((pOutdatedEntry) => !isUpAble(pOutdatedEntry));
    if (lNotUpdated.length > 0) {
        lReturnValue += `Up'em found these packages were outdated, but did not update them because of policies:${EOL}${EOL}${lNotUpdated
            .map((pOutdatedEntry) => `${pOutdatedEntry.package.padEnd(lMaxPackageLength)}${pOutdatedEntry.current.padEnd(lMaxTargetLength)}${pOutdatedEntry.type?.padEnd(lMaxTypeLength) ?? ""} (policy: ${pOutdatedEntry.policy})`)
            .join(EOL)}${EOL}${EOL}`;
    }
    return lReturnValue;
}
function determineOutdated(pOutdatedObject, pPackageObject) {
    const lOutdatedObject = pOutdatedObject.length <= 0 ? {} : JSON.parse(pOutdatedObject);
...
export default function upem(pPackageInputFileName, pOutdatedJSON, pPackageOutputFileName = pPackageInputFileName, pOptions = {}) {
    try {
        ...
        try {
            if (!pOptions.dryRun) {
                writeFileSync(pPackageOutputFileName, JSON.stringify(updateManifest(lPackageObject, lOutdatedResult.outdatedList.filter(isUpAble), pOptions), null, INDENT));
            }
            return {
                OK: true,
